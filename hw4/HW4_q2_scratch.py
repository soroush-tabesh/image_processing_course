...
def draw_polygons(src, triangles_indexed, points):
    src = src.copy()
    for tr in triangles_indexed:
        tt = points[list(tr)]
        cv.polylines(src, [tt], 1, (0, 0, 255))
    return src


tries = triangulate(img_src, points_src)
imshow(draw_polygons(img_src, tries, points_src), draw_polygons(img_tar, tries, points_tar))
...
video_writer = cv.VideoWriter(f'../video/hw4-2-{time.time()}.avi', cv.VideoWriter_fourcc(*'MJPG'),
                              frame_rate,
...
# process = (
#     ffmpeg
#         .input('pipe:', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(img_src.shape[1], img_src.shape[0]))
#         .output(f'./video/hw4-2-{time.time()}.mp4', pix_fmt='yuv420p', r=frame_rate, crf=10)
#         .overwrite_output()
#         .run_async(pipe_stdin=True)
# )
# process.stdin.write(
#     cv.cvtColor(img_res, cv.COLOR_BGR2RGB)
#         .astype(np.uint8)
#         .tobytes()
# )
# process.stdin.close()
