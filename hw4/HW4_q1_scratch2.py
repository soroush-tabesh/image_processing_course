...
# unit tests
#
# frame = np.zeros((sz_smp, 2 * sz_smp - sz_ovlp, 3), dtype=np.uint8)
#
# smp1 = draw_random_sample(texture[0], sz_smp)
# frame[:sz_smp, :sz_smp] = smp1
# smp2 = draw_random_similar_sample(texture, cv.cvtColor(frame[:, sz_smp - sz_ovlp:], cv.COLOR_RGB2GRAY), texture_g,
#                                   sz_ovlp, False, True)
#
# # imshow(smp1, smp2)
# msk = get_mask_cut(frame[:, sz_smp - sz_ovlp:], smp2, sz_ovlp)[:, :, None]
# frame[:, sz_smp - sz_ovlp:] = frame[:, sz_smp - sz_ovlp:] * msk + (1 - msk) * smp2
# imshow(frame)
...
for i, j in np.ndindex(*cnt_smp):
    ...
    # imshow(msk, smp_old, smp_new, smp)
